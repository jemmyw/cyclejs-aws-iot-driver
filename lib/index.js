...
function makeIotDriver(options) {
  ...
  */
  var subscribeTopic = function subscribeTopic(runStreamAdapter) {
    return function (topic) {
      subscriptionSource$.onNext(topic);

      var topicStream$ = messages$.filter(_ramda2.default.propEq('topic', topic));

      return runStreamAdapter.adapt(topicStream$, _rxAdapter2.default.streamSubscribe);
    };
  };
  ...
  */
  function iotDriver(events$, runStreamAdapter) {
    ...
    var responses$$ = runStreamAdapter.adapt(messages$.map(_ramda2.default.identity).share(), _rxAdapter2.default.streamSubscribe);
  ...
  iotDriver.streamAdapter = _rxAdapter2.default;
  return iotDriver;
}
